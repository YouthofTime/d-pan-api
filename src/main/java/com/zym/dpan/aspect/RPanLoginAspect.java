package com.zym.dpan.aspect;

import com.zym.dpan.utils.Constant;
import com.zym.dpan.utils.JwtUtil;
...
public class RPanLoginAspect {
     ...
     * 切点入口
     */
    private final String POINT_CUT = "@annotation(com.zym.dpan.annotation.NeedLogin)";
    ...
    public Object loginAuth(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        if (!checkAndSaveUserId(request)) {
            ...
            return R.fail(Constant.ResponseCode.NEED_LOGIN.getCode(), Constant.ResponseCode.NEED_LOGIN.getDesc());
        }
    ...
    private boolean checkAndSaveUserId(HttpServletRequest request) {
        ...
        Object userId = JwtUtil.analyzeToken(token, Constant.LOGIN_USER_ID);
        if (Objects.isNull(userId)) {
            return false;
        }
        Object redisValue = stringRedisTemplate.opsForValue().get(Constant.USER_LOGIN_PREFIX + userId);
