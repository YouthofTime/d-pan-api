...
import com.zym.dpan.user.dao.RPanUserSearchHistoryMapper;
import com.zym.dpan.user.entity.RPanUserSearchHistory;
...
public class UserSearchHistoryServiceImpl implements IUserSearchHistoryService {
    ...
    private void stickSearchContent(String searchContent, Long userId) {
        if (rPanUserSearchHistoryMapper.updateUpdateTimeBySearchContentAndUserId(searchContent, userId) != Constant.ONE_INT) {
            throw new RRException("置顶搜索历史信息失败");
        }
    }
    ...
    private void saveSearchContent(String searchContent, Long userId) {
        if (rPanUserSearchHistoryMapper.insertSelective(assembleRPanUserSearchHistoryEntity(searchContent, userId)) != Constant.ONE_INT) {
            throw new RRException("保存搜索历史信息失败");
        }
    }
